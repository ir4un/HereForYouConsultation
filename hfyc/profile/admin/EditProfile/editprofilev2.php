...
if (isset($_SESSION['id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "UPDATE `admin` SET AdminName = '$name', AdminAge = '$age', AdminAddress = '$address', AdminDescription = '$description' WHERE UserID = " . $_SESSION['id'];

        $sql2 = "UPDATE users SET UserEmail = '$email' WHERE UserID = " . $_SESSION['id'];

        if (mysqli_query($conn, $sql) && mysqli_query($conn, $sql2)) {
            echo "<script>location.href = '../profile/adminprofilev2.php?info=editprofilesuccess'</script>";
        } else {
            echo "<script>location.href = '../profile/adminprofilev2.php?error=failtoupdate'</script>";
        }
    }
        ...
        if (in_array($extension, $allowed_list)) {
            if ($fileerror == 0) {
                if ($filesize < 10000000) {
                    ...
                    $sql = "UPDATE `admin` SET AdminImgStatus = 1 WHERE UserID = " . $_SESSION['id'];
