...
if (isset($_SESSION['id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "UPDATE normal_user SET NormalUserName = '$name', NormalUserAge = '$age', NormalUserAddress = '$address', NormalUserDescription = '$description' WHERE UserID = " . $_SESSION['id'];

        $sql2 = "UPDATE users SET UserEmail = '$email' WHERE UserID = " . $_SESSION['id'];

        if (mysqli_query($conn, $sql) && mysqli_query($conn, $sql2)) {
            echo "<script>location.href = '../Profile/userprofilev2.php?info=editprofilesuccess'</script>";
        } else {
            echo "<script>location.href = '../Profile/userprofilev2.php?error=failtoupdate'</script>";
        }
    }
        ...
        if (in_array($extension, $allowed_list)) {
            if ($fileerror == 0) {
                if ($filesize < 10000000) {
                    ...
                    $sql = "UPDATE normal_user SET NormalUserImgStatus = 1 WHERE UserID = " . $_SESSION['id'];
